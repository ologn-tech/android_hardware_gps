...
#define DEFAULT_GPS_DEVICE    "ttyS3"
#define DEFAULT_GPS_TTYBAUD   "9600"
#define DEFAULT_GPS_MAX_RATE  "1000"
#define DEFAULT_GPS_TIME_SYNC "60"
...
gps_state_init( GpsState*  state, GpsCallbacks* callbacks )
{
    ...
    if (property_get("persist.vendor.gps.device", prop, DEFAULT_GPS_DEVICE) == 0) {
    ...
    if (property_get("persist.vendor.gps.max_rate", prop, DEFAULT_GPS_MAX_RATE) != 0)
    {
    ...
    time_sync = false;
    if (property_get("persist.vendor.gps.time_sync", prop, DEFAULT_GPS_TIME_SYNC) != 0)
    {
        time_sync = atol(prop);
    }
    ...
    if ( isatty( state->fd ) ) {
        ...
        property_get("persist.vendor.gps.ttybaud", prop, DEFAULT_GPS_TTYBAUD);
